...
```Scala
case class Flock(var seagulls:Int){

  def conjoin(other:Flock) = {
   this.seagulls = this.seagulls + other.seagulls
  this
  }
def breed(other:Flock) ={
      this.seagulls = this.seagulls * other.seagulls
        this
      }
}
object Flock{
val flockA = new Flock(4);
val flockB = new Flock(2);
val flockC = new Flock(0);
val result = flockA
  .conjoin(flockC)
  .breed(flockB)
...
```Scala
case class Flock( seagulls:Int)
val conjoin:(Flock,Flock) => Flock = (flockX:Flock, flockY:Flock) => Flock(flockX.seagulls + flockY.seagulls)
val breed:(Flock,Flock) => Flock = (flockX:Flock, flockY:Flock) => Flock(flockX.seagulls * flockY.seagulls)

val flockA = Flock(4)
val flockB = Flock(2)
val flockC = Flock(0)
val result = conjoin(breed(flockB, conjoin(flockA, flockC)), breed(flockA, flockB))
// 16
```
